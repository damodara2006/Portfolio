...
import resume from "/home/pdp28/Projects/Portfolio/frontend/public/Damodara prakash Resume.pdf";
...
function Home() {
  ...
  function handleClick(e) {
    ...
    axios
      .post("https://formsubmit.co/damodara2006@gmail.com", { message, email })
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  return (
    <html className="scroll-smooth ">
      <div id="home" className="bg-black flex flex-col !scroll-smooth">
        {/* Fixed Navbar */}
        <div className="fixed top-0 left-0 w-full bg-[linear-gradient(140deg,_#B7B1F2,_#F8BDEB)] shadow-md z-50">
        ...
        <div className="mt-16 flex flex-col items-center">
          ...
          <div className="flex justify-center items-center w-full">
            <div className="relative sm:w-64 mt-5 rounded-full flex justify-center border w-72 h-72 border-white text-white overflow-hidden">
          ...
          <p id="achievements" className="text-center text-3xl text-rose-500 font-extrabold font-mono mt-9">
            Achievements
          </p>
          <div className="flex justify-items-center gap-8 px-5 justify-evenly mt-7 flex-wrap">
            {[nptel, MKCE, Learnathon, kongu].map((img, index) => (
              <div key={index} className="border border-white max-w-80 max-h-80 rounded-2xl shadow-xl shadow-gray-600 transition-transform duration-500 ease-in-out overflow-hidden">
          ...
          <div className="mt-10">
            <a
              ...
              className="text-white bg-blue-500 px-4 py-2 rounded-md hover:bg-blue-700 transition-all"
            >
              Download Resume
            </a>
          </div>
          ...
          {/* Contact Section */}
          <div id="contact" className="mt-10 text-white text-center text-3xl flex flex-col mb-4">
            Contact
            <form action={"https://formsubmit.co/damodara2006@gmail.com"} method="POST">
              <input type="text" name="email" className="border rounded-sm mb-1 text-sm py-1.5 w-80 pl-5" placeholder="Enter your email" value={email} onChange={(e) => setemail(e.target.value)} />
              <br />
              <input type="text" name="name" className="border rounded-sm mb-1 text-sm py-1.5 w-80 pl-5" placeholder="Message me" value={message} onChange={(e) => setmessage(e.target.value)} />
              <br />
              <button className="text-sm border px-3 rounded-sm hover:bg-gray-800 transition-all" type="submit">
                Submit
              </button>
            </form>
          </div>
